...
def get_inbounds(driver, selectors):
    try:
        ...
        click = None
        messages = []
        if driver.switch_to.active_element == driver.find_element_by_xpath(selectors["search"]) or driver.switch_to.active_element == driver.find_element_by_xpath(selectors["message"]):
            click = True
        try:
            if click:
